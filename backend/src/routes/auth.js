...
router.post('/login', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'SELECT id, name, email, password_hash, role, company_id, must_change_password, active FROM users WHERE email = $1 AND deleted_at IS NULL',
        ...
        const token = jwt.sign(
            { id: user.id, name: user.name, email: user.email, role: user.role, companyId: user.company_id },
